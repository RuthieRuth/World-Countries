...
let lastFetchTime = 0;
const CACHE_EXPIRY = 30000; // 30 seconds to prevent unnecessary API calls.aka, if the last fetch was less than 30 seconds ago, it will be refreshed from the cache
...
export const favouritesApi = {
  ...
  async getFavorites(useCache = true): Promise<CountryFavorite[]> {
    const now = Date.now(); //explain again
   ...
   */
  async addFavourite(country: Country): Promise<CountryFavorite> {
    const { data, error } = await supabase
      .from("country_favorites")
      .insert([
        {
   ...
   */
  async removeFavourite(countryName: string): Promise<void> {
